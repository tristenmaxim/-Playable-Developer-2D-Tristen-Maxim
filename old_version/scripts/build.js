...
(function() {
    console.log('[AssetLoader Patch] Начало применения патча...');
    
    // Ждем загрузки PIXI, если он еще не загружен (для CDN)
    function applyPatch() {
        if (typeof EMBEDDED_ASSETS === 'undefined') {
            console.error('[AssetLoader Patch] EMBEDDED_ASSETS не найдены!');
            return false;
        }
        
        if (typeof AssetLoader === 'undefined') {
            console.error('[AssetLoader Patch] AssetLoader класс не найден!');
            return false;
        }
        
        if (typeof PIXI === 'undefined') {
            console.error('[AssetLoader Patch] PIXI не найден!');
            return false;
        }
        
        if (typeof PIXI.Texture === 'undefined') {
            console.error('[AssetLoader Patch] PIXI.Texture не найден!');
            return false;
        }
        
        console.log('[AssetLoader Patch] Все проверки пройдены, применяю патч...');
    
    // Переопределяем метод load
    AssetLoader.prototype.load = async function() {
        console.log('[AssetLoader] Загрузка встроенных ассетов...');
        
        // Убеждаемся, что loadedAssets инициализирован
        ...
        // Загружаем все ассеты (используем Promise.all для параллельной загрузки)
        const loadPromises = [];
        ...
        Object.keys(assetMapping).forEach(fileName => {
            ...
            if (mimeType.startsWith('image/')) {
                ...
                console.log(\`[AssetLoader] Загрузка текстуры \${alias} из data URI...\`);
                
                // Используем Promise для загрузки изображения (так как это асинхронная операция)
                const loadPromise = new Promise((resolve, reject) => {
                    ...
                    img.onerror = (error) => {
                        console.error(\`[AssetLoader] ✗ Ошибка загрузки изображения \${alias}:\`, error);
                        this.loadedAssets[alias] = { texture: PIXI.Texture.EMPTY };
                        resolve(PIXI.Texture.EMPTY);
                    };
                ...
                // Сохраняем промис для ожидания загрузки
                if (!this._texturePromises) {
                    this._texturePromises = [];
                }
                this._texturePromises.push(loadPromise);
        ...
        if (this._texturePromises && this._texturePromises.length > 0) {
            await Promise.all(this._texturePromises);
            this._texturePromises = [];
        }
        
        console.log('[AssetLoader] Все встроенные ассеты загружены');
        console.log('[AssetLoader] Загружено ассетов:', Object.keys(this.loadedAssets).length);
    ...
    console.log('[AssetLoader Patch] Патч успешно применен!');
    return true;
    }
    
    // Пробуем применить патч сразу
    if (!applyPatch()) {
        // Если не получилось, ждем загрузки PIXI (для CDN)
        console.log('[AssetLoader Patch] Ожидание загрузки PIXI...');
        let attempts = 0;
        const checkPIXI = setInterval(() => {
            attempts++;
            if (typeof PIXI !== 'undefined' && typeof PIXI.Texture !== 'undefined') {
                clearInterval(checkPIXI);
                console.log('[AssetLoader Patch] PIXI загружен, применяю патч...');
                applyPatch();
            } else if (attempts > 50) {
                clearInterval(checkPIXI);
                console.error('[AssetLoader Patch] Таймаут ожидания PIXI (5 секунд)!');
            }
        }, 100);
    }
})();
